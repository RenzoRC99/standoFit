package com.standofit.back.training.planning.infrastructure.mapper;
...
public class DatabaseExceptionMapper {
    ...
    public WorkoutInfrastructureException map(DataAccessException e, WorkoutInfrastructureErrors defaultError) {
        Throwable cause = e.getMostSpecificCause();

        return mapSqlException(cause)
                .or(() -> mapTimeoutException(cause))
                .or(() -> mapConstraintViolation(e))
                .orElse(new WorkoutInfrastructureException(defaultError.getMessage() + ": " + cause.getMessage()));
    }
    ...
    private Optional<WorkoutInfrastructureException> mapTimeoutException(Throwable cause) {
        if (cause instanceof QueryTimeoutException) {
            return Optional.of(new WorkoutInfrastructureException(WorkoutInfrastructureErrors.DB_TIMEOUT.getMessage()));
        }
        return Optional.empty();
    }
    ...
    private Optional<WorkoutInfrastructureException> mapConstraintViolation(DataAccessException e) {
        if (e instanceof DataIntegrityViolationException) {
            return Optional.of(new WorkoutInfrastructureException(WorkoutInfrastructureErrors.DB_CONSTRAINT_VIOLATION.getMessage()));
        }
        return Optional.empty();
    }
}