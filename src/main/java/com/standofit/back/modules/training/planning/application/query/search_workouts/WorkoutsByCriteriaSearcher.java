...
public class WorkoutsByCriteriaSearcher extends PlanningUseCase {
  ...
  public WorkoutsByCriteriaSearcher(WorkoutRepository repository, WorkoutDtoMapper mapper, ApplicationEventBus applicationEventBus) {
    super(repository, mapper, applicationEventBus);
  }

  public PagedResult<WorkoutDto> search(Filters filters, Order order, int page, int pageSize) {
    try {
      Criteria criteria = Criteria.fromValues(
      ...
      var workouts = repository.searchByCriteria(criteria).stream()
          .map(mapper::toDto)
          .toList();
      ...
      publishEvent(PlanningActivityEvent.success("workouts.searched", null, "Searched workouts with filters"));
      return result;
    } catch (Exception e) {
      publishEvent(PlanningActivityEvent.failure("workouts.searched", null, "Failed to search workouts", e.getMessage()));
      throw e;
    }
  }
}