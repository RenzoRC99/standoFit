...
import com.standofit.back.modules.training.execution.application.query.get_exercise_logs.GetExerciseLogsQuery;
...
public class SessionApiController implements ApiApi {
    ...
    public SessionApiController(ApplicationBusFacade bus) {
        this.bus = bus;
    }
    ...
    @Override
    public ResponseEntity<List<ExerciseLogDTO>> getExerciseLogs(UUID sessionId) {
        try {
            List<ExerciseLogDTO> payload = bus.ask(new GetExerciseLogsQuery(new SessionId(sessionId)));
            if (payload.isEmpty()) {
                return ResponseEntity.noContent().build();
            }
            return ResponseEntity.ok(payload);
    ...
    public ResponseEntity<Void> addExerciseLog(UUID sessionId, AddExerciseLogRequest body) {
        try {
            bus.execute(new AddExerciseLogCommand(
                    new SessionId(sessionId),
                    body.getLogId() != null ? new ExerciseLogId(body.getLogId()) : new ExerciseLogId(UUID.randomUUID()),
    ...
    public ResponseEntity<Void> updateExerciseLogSets(UUID sessionId, UUID logId, UpdateSetsRequest body) {
        try {
            bus.execute(new UpdateExerciseLogSetsCommand(new SessionId(sessionId), new ExerciseLogId(logId), new ExerciseLogSets(body.getSets())));
            return ResponseEntity.noContent().build();
        } catch (IllegalArgumentException e) {
            throw new SessionNotFoundException(sessionId);
        }
    ...
    public ResponseEntity<Void> updateExerciseLogReps(UUID sessionId, UUID logId, UpdateRepsRequest body) {
        try {
            bus.execute(new UpdateExerciseLogRepsCommand(new SessionId(sessionId), new ExerciseLogId(logId), new ExerciseLogReps(body.getReps())));
            return ResponseEntity.noContent().build();
        } catch (IllegalArgumentException e) {
            throw new SessionNotFoundException(sessionId);
        }
    ...
    public ResponseEntity<Void> updateExerciseLogWeight(UUID sessionId, UUID logId, UpdateWeightRequest body) {
        try {
            bus.execute(new UpdateExerciseLogWeightCommand(new SessionId(sessionId), new ExerciseLogId(logId), new ExerciseLogWeight(body.getWeight())));
            return ResponseEntity.noContent().build();
        } catch (IllegalArgumentException e) {
            throw new SessionNotFoundException(sessionId);
        }
    ...
    public ResponseEntity<Void> updateSessionNotes(UUID sessionId, UpdateNotesRequest body) {
        try {
            bus.execute(new UpdateSessionNotesCommand(new SessionId(sessionId), new WorkoutSessionNotes(body.getNotes())));
            return ResponseEntity.noContent().build();
        } catch (IllegalArgumentException e) {
            throw new SessionNotFoundException(sessionId);
        }
