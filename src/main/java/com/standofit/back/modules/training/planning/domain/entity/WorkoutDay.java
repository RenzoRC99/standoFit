...
public final class WorkoutDay {
    ...
    public static WorkoutDay create(
            WorkoutDayId id, WorkoutDayName name, List<WorkoutExercise> exercises) {
        return new WorkoutDay(id, name, isNullOrEmpty(exercises) ? List.of() : exercises);
    }
    ...
    WorkoutDay updateExercises(List<WorkoutExercise> updatedExercises) {
        ...
        List<WorkoutExerciseId> idsToUpdate =
                updatedExercises.stream().map(WorkoutExercise::getId).toList();
    ...
    private Set<WorkoutExerciseId> getExerciseIds() {
        return this.exercises.stream().map(WorkoutExercise::getId).collect(Collectors.toSet());
    }
    ...
    private void validateIdsExist(List<WorkoutExerciseId> ids) {
        ids.stream()
                ...
                .ifPresent(
                        id -> {
                            throw new WorkoutDomainException(
                                    WorkoutDomainErrors.EXERCISE_ID_NOT_FOUND.getMessage());
                        });
    }
    ...
    private void ensureNoDuplicateExerciseIds(List<WorkoutExercise> exercises) {
        ...
        long uniqueIds = exercises.stream().map(WorkoutExercise::getId).distinct().count();
