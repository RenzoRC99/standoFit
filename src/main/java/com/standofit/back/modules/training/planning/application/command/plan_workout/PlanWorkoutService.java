...
public class PlanWorkoutService extends PlanningUseCase {
    ...
    public PlanWorkoutService(WorkoutRepository repository, WorkoutDtoMapper mapper, ApplicationEventBus applicationEventBus) {
    ...
    public UUID plan(PlanWorkoutCommand command) {
        try {
            ...
            for (PlanWorkoutCommand.DayInput dayInput : command.days()) {
                ...
                WorkoutDay day =
                        WorkoutDay.create(
                                new WorkoutDayId(UUID.randomUUID()), new WorkoutDayName(dayInput.name()), exercises);
                days.add(day);
            }
            ...
            publishEvent(PlanningActivityEvent.success("workout.planned", id.toString(), "Planned workout: " + command.name()));
            return id;
        } catch (Exception e) {
            publishEvent(PlanningActivityEvent.failure("workout.planned", null, "Failed to plan workout", e.getMessage()));
            throw e;
        }
    }
}
