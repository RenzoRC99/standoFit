...
class WorkoutRestApiIntegrationTest {
    ...
    private String createWorkoutWithDays(String name, List<String> dayNames) throws Exception {
        var request = new PlanWorkoutRequest(
                ...
                "Description",
                dayNames.stream()
                        .map(d -> new PlanWorkoutRequest.DayRequest(d, List.of()))
                        .toList());
    ...
    class RemoveDay {
        ...
        void shouldRemoveDayFromWorkout() throws Exception {
            ...
            var workout = mockMvc.perform(get("/api/workouts/" + workoutId))
                    .andExpect(status().isOk())
                    .andReturn();
            String dayId = objectMapper.readTree(workout.getResponse().getContentAsString())
                    .get("days").get(0).get("id").asText();

            mockMvc.perform(delete("/api/workouts/" + workoutId + "/days/" + dayId))
                    .andExpect(status().isNoContent());
    ...
    class DuplicateWorkout {
        ...
        void shouldDuplicateWorkout() throws Exception {
            ...
            String newId = mockMvc.perform(post("/api/workouts/" + workoutId + "/duplicate")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content("{\"newName\": \"Copy\"}"))
                    .andExpect(status().isCreated())
                    .andReturn().getResponse().getContentAsString().replace("\"", "");

            mockMvc.perform(get("/api/workouts/" + newId))
    ...
    class ArchiveWorkout {
        ...
        void shouldArchiveWorkout() throws Exception {
            ...
            mockMvc.perform(post("/api/workouts/" + workoutId + "/archive"))
                    .andExpect(status().isNoContent());
        }
    }
